...
const {SERVER_ERROR, EVENT_NOT_FOUND} = require("./util/events.dict.errors")
...
const create = async (eventData) => {
  try {
    ...
    throw error.errorHandle ? error : errorHandle(SERVER_ERROR, error)
...
const getOne = async (eventId) => {
  ...
  try {
    if (!moongose.Types.ObjectId.isValid(eventId)) {
      throw error.handler(EVENT_NOT_FOUND)
    }
    const eventObject = await Event.findById(eventId)
    if (eventObject) return eventObject
    throw errorHandle(EVENT_NOT_FOUND)
  } catch (error) {
    throw error.handler ? error : errorHandle(SERVER_ERROR, error)
  }
